...
public class BalanceSheetService {
    ...
    public BalanceSheetSummaryDTO getBalanceSheet(LocalDate startDate, LocalDate endDate) {
        ...
        for (Object[] row : results) {
            ...
            // --- 借貸方向調整 ---
            if (parentId >= 2000 && parentId < 3000) { 
                // 負債：貸方應為正，借方為負
                balance = balance.negate(); 
            } else if (parentId >= 3000 && parentId < 4000 && parentId != 3600) { 
                // 權益 (排除本期損益)：貸方應為正，借方為負
                balance = balance.negate(); 
            }
            // 資產不用調整，因為借方為正數即可
            ...
            if (parentId >= 1000 && parentId < 2000) { // 資產
                ...
                if (parentId < 1500) currentAssetsTotal = currentAssetsTotal.add(balance);
                else nonCurrentAssetsTotal = nonCurrentAssetsTotal.add(balance);
            } else if (parentId >= 2000 && parentId < 3000) { // 負債
                totalLiabilities = totalLiabilities.add(balance);
                if (parentId < 2500) currentLiabilitiesTotal = currentLiabilitiesTotal.add(balance);
                else nonCurrentLiabilitiesTotal = nonCurrentLiabilitiesTotal.add(balance);
            } else if (parentId >= 3000 && parentId < 4000 && parentId != 3600) { // 權益 (不含本期損益)
                totalEquity = totalEquity.add(balance);
                if (parentId == 3100) capitalTotal = capitalTotal.add(balance);
                else if (parentId == 3600) retainedEarningsTotal = retainedEarningsTotal.add(balance);
            }
        }
